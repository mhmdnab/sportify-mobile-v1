...
export function LoginScreen() {
  ...
  const handleAppleLogin = async () => {
    try {
      ...
      if (err?.code !== 'ERR_REQUEST_CANCELED') {
        Alert.alert('Apple Sign-In Failed', err?.response?.data?.message || 'Something went wrong.');
      }
    } finally {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const currentUser = useAuthStore.getState().user;
      const target = currentUser?.owner
        ? "OwnerApp"
        : currentUser?.manager
          ? "ManagerApp"
          : currentUser?.coach
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Google */}
            <TouchableOpacity
              style={[styles.socialBtn, (!request || googleLoading) && { opacity: 0.6 }]}
              activeOpacity={0.8}
            ...
            >
              ...
              <Text style={styles.socialBtnText}>{t("auth.loginWithGoogle")}</Text>
            </TouchableOpacity>

            {/* Apple — iOS only */}
            {Platform.OS === 'ios' && (
              <TouchableOpacity
                style={[styles.socialBtn, styles.appleBtn, appleLoading && { opacity: 0.6 }]}
                activeOpacity={0.8}
