...
function CoachCard({
  ...
  return (
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <Text
          style={[cardStyles.name, { color: tc.textPrimary }]}
          numberOfLines={1}
        >
        ...
        >
          ...
          {reviewCount > 0 && (
            <Text style={[cardStyles.ratingCount, { color: tc.textHint }]}>
              ({reviewCount})
            </Text>
          )}
        </View>
      ...
      <View style={cardStyles.right}>
        <Text
          style={[cardStyles.rate, { color: isDark ? "#A2B8FF" : "#0B1A3E" }]}
        >
          ${coach.hourlyRate ?? 0}
        </Text>
        ...
        <View
          style={[
            cardStyles.bookBtn,
            { backgroundColor: isDark ? "#162B5C" : "#0B1A3E" },
          ]}
        >
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        isSelected && allChipStyles.selected,
      ]}
    >
      <Ionicons
        name="apps"
        size={16}
        color={isSelected ? "#fff" : tc.textPrimary}
        style={{ marginRight: 6 }}
      />
      <Text
        style={[
          ...
          isSelected && allChipStyles.selectedLabel,
        ]}
      >
        All
      </Text>
...
export function CoachesListScreen({ navigation }: Props) {
  ...
  const fetchCoaches = useCallback(
    async (q: string, sportId: number | null) => {
      try {
  ...
  const handleSearch = (text: string) => {
    ...
    searchTimer.current = setTimeout(
      () => fetchCoaches(text, selectedSportId),
      400,
    );
  };
  ...
  const headerEl = (
    <>
      ...
      <View style={[headerStyles.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={[headerStyles.title, { color: tc.textPrimary }]}>
          Coaches
        </Text>
        <View style={{ width: 36 }} />
      </View>
  ...
  return (
    <View
      style={[styles.root, { backgroundColor: isDark ? "#060F28" : "#F4F6FB" }]}
    >
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={onRefresh}
              tintColor={isDark ? "#A2B8FF" : "#0B1A3E"}
            />
          }
          renderItem={({ item }) => (
            <CoachCard
