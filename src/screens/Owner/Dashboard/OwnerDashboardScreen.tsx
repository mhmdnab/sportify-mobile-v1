...
function HeroSection({ name, isDark }: { name: string; isDark: boolean }) {
  ...
  return (
    <LinearGradient
      colors={
        isDark
          ? ["#060F28", "#0F2048", "#162B5C"]
          : ["#0B1A3E", "#132452", "#1A3070"]
      }
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 1 }}
    ...
    <Animated.View entering={FadeInDown.delay(80).springify()}>
      ...
      >
        <View style={revStyles.left}>
          <Text style={[revStyles.label, { color: tc.textHint }]}>
            Today's Revenue
          </Text>
        ...
        <View style={revStyles.right}>
          ...
          <Text style={[revStyles.trendLabel, { color: tc.textHint }]}>
            Last 7 bookings
          </Text>
        </View>
      </LinearGradient>
    ...
    >
      ...
      >
        <ProgressRing
          ...
          iconColor={ringColor}
        />
        <Text style={[metStyles.value, { color: tc.textPrimary }]}>
          {value}
        </Text>
...
function UpNextCard({
  ...
  const venueName =
    reservation.slot?.availability?.venue?.name || t("owner.venue");
  const userName = reservation.user?.name || t("owner.user");
  const start = reservation.slot?.startTime
    ? formatAMPM(reservation.slot.startTime)
    : "";
  const end = reservation.slot?.endTime
    ? formatAMPM(reservation.slot.endTime)
    : "";
  const startMin = reservation.slot?.startTime
    ? toMin(reservation.slot.startTime)
    : 0;
    ...
    >
      ...
      >
        ...
        <View style={upStyles.body}>
          ...
          <Text style={[upStyles.time, { color: tc.textPrimary }]}>
            {start}
          </Text>
          <Text
            style={[upStyles.name, { color: tc.textPrimary }]}
            numberOfLines={1}
          >
            {userName}
          </Text>
          <View style={upStyles.meta}>
            ...
            <Text
              style={[upStyles.metaText, { color: tc.textSecondary }]}
              numberOfLines={1}
            >
              {venueName}
            </Text>
            {end ? (
              <>
                ...
                <Text style={[upStyles.metaText, { color: tc.textSecondary }]}>
                  {start} – {end}
                </Text>
              </>
            ) : null}
  ...
  return (
    ...
    >
      {pills.map((p, i) => (
        ...
        >
          <View
            style={[pillStyles.iconBox, { backgroundColor: `${p.color}18` }]}
          >
            <Ionicons name={p.icon as any} size={16} color={p.color} />
          </View>
          <Text style={[pillStyles.val, { color: tc.textPrimary }]}>
            {p.value}
          </Text>
          <Text style={[pillStyles.lbl, { color: tc.textHint }]}>
            {p.label}
          </Text>
        </View>
      ))}
  ...
  return (
    <View style={actStyles.row}>
      ...
      <View style={actStyles.info}>
        <Text
          style={[actStyles.user, { color: tc.textPrimary }]}
          numberOfLines={1}
        >
          {userName}
        </Text>
        <Text
          style={[actStyles.detail, { color: tc.textSecondary }]}
          numberOfLines={1}
        >
          {venueName}
          {slotTime ? ` · ${slotTime}` : ""}
      ...
      >
        ...
        <Animated.View entering={FadeInDown.delay(340).springify()}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={[styles.myVenuesLabel, { color: tc.textPrimary }]}>
                My Venues
              </Text>
              <Text style={[styles.myVenuesHint, { color: tc.textHint }]}>
                {ownVenues.length} venue{ownVenues.length !== 1 ? "s" : ""} ·
                tap to browse & book
              </Text>
            </View>
