...
/** Returns minutes-since-midnight for "HH:MM:SS" */
...
export function OwnerScheduleScreen() {
  const { t } = useTranslation();
  const tc = useThemeColors();
  const isDark = useThemeStore((s) => s.isDark);
  const { allReservations, isLoadingSchedule, fetchForSchedule } = useOwnerReservationsStore();
  ...
  // Filter and sort reservations for selected date
  ...
  // Determine "current" item: selected date is today and now falls within slot time range
