...
export function ReservationScreen({ route, navigation }: Props) {
  ...
  const { createReservation, isLoading: bookingLoading } =
  ...
  useEffect(() => {
    ...
    api
      .get<any>("/coaches", {
        params: { venueId, day: dayName, page: 1, limit: 50 },
      })
      .then((res) => {
        ...
        if (
          list.length > 0 &&
          !list.find((c: any) => c.id === selectedCoachId)
        ) {
  ...
  const hasSlots = (date: Date) => {
    ...
    const avail = currentVenue?.availability?.find(
      (a) => a.day === dow && a.isOpen,
    );
  ...
  const totalVenuePrice = selectedSlots.reduce(
    (acc, s) => acc + (s.price ?? 0),
    0,
  );
  const totalDurationHours = selectedSlots.reduce(
    (acc, s) => acc + slotDuration(s),
    0,
  );
  const coachFee =
    withCoach && selectedCoach
      ? (selectedCoach.hourlyRate ?? 0) * totalDurationHours
      : 0;
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      const reservation = await createReservation({
        ...
        ...(extraSlots.length > 0 && {
          additionalSlotIds: extraSlots.map((s) => s.id),
        }),
        slotDate: formatSlotDate(selectedDate),
        notes: notes || undefined,
        ...(withCoach &&
          selectedCoachId && { withCoach: true, coachId: selectedCoachId }),
      });
    ...
    <View style={[styles.container, { backgroundColor: tc.screenBg }]}>
      ...
      <View
        style={[
          styles.header,
          {
            ...
            backgroundColor: isDark ? "#040B1E" : colors.navy,
          },
        ]}
      >
        <TouchableOpacity
      ...
      >
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  ...
                  isSelected && {
                    backgroundColor: isDark ? "#1D4ED8" : colors.navy,
                    borderColor: isDark ? "#3B6FE0" : colors.navy,
                  },
                ]}
                activeOpacity={0.7}
              >
                ...
                <View style={styles.dotRow}>
                  {available && (
                    <View
                      style={[
                        styles.availDot,
                        isSelected && styles.availDotSelected,
                      ]}
                    />
                  )}
                </View>
        ...
        <View style={styles.reservationNameSection}>
          ...
          <View
            style={[styles.reservationNameCard, { backgroundColor: tc.cardBg }]}
          >
            <FontAwesome6
              name="people-group"
              size={16}
            ...
            />
            <Text
              style={[styles.reservationNameText, { color: tc.textPrimary }]}
            >
              {user?.name || "Guest"}
            </Text>
          </View>
        ...
        <Text style={[styles.sectionLabel, { color: tc.textPrimary }]}>
          ...
          {selectedSlots.length > 0 && (
            <Text
              style={{ color: isDark ? "#A2B8FF" : colors.navy, fontSize: 13 }}
            >
              {" "}
        ...
        {availableSlots.length === 0 ? (
          <View style={styles.noSlotsContainer}>
            ...
            <Text style={[styles.noSlotsText, { color: tc.textHint }]}>
              No available slots for this day
            </Text>
            <Text style={[styles.noSlotsHint, { color: tc.textHint }]}>
              Look for dates with a green dot above
            </Text>
          </View>
        ) : (
            ...
            <View style={styles.coachToggleRow}>
              <View style={styles.coachToggleLeft}>
                <FontAwesome6
                  name="people-group"
                  size={16}
                  color={isDark ? "#A2B8FF" : "#0B1A3E"}
                />
                <Text
                  style={[styles.coachToggleLabel, { color: tc.textPrimary }]}
                >
                  Book with Coach
                </Text>
              </View>
              <Switch
                ...
                trackColor={{
                  true: isDark ? colors.navyLight : "#0B1A3E",
                  false: isDark ? "#2A3A5C" : "#888",
                }}
                thumbColor="#fff"
              />
            </View>
                ...
                {availableCoaches.map((coach) => {
                  ...
                  return (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.coachItem,
                        isCoachSelected && styles.coachItemSelected,
                      ]}
                    >
                      <View style={styles.coachItemLeft}>
                        <View
                          style={[
                            styles.coachAvatar,
                            {
                              backgroundColor: isCoachSelected
                                ? isDark
                                  ? colors.navyLight
                                  : "#0B1A3E"
                                : isDark
                                  ? "rgba(162,184,255,0.12)"
                                  : "rgba(11,26,62,0.1)",
                            },
                          ]}
                        >
      ...
      <View
        style={[
          styles.bottomBar,
          { paddingBottom: insets.bottom + 12, backgroundColor: tc.cardBg },
        ]}
      >
        {selectedSlots.length > 0 && (
        ...
        <TouchableOpacity
          style={[
            styles.bookNowBtn,
            { backgroundColor: isDark ? "#1D4ED8" : colors.navy },
            selectedSlots.length === 0 && styles.bookNowBtnDisabled,
          ]}
      ...
      >
        ...
        >
          ...
          <View style={[styles.modalSheet, { backgroundColor: tc.cardBg }]}>
            <View
              style={[styles.modalHandle, { backgroundColor: tc.border }]}
            />
            ...
            <View style={styles.summarySection}>
              ...
              <SummaryRow
                label={
                  selectedSlots.length > 1
                    ? `Venue subtotal (${selectedSlots.length} slots)`
                    : "Venue fee"
                }
            ...
            <TextInput
              style={[
                styles.notesInput,
                { backgroundColor: tc.surface, color: tc.textPrimary },
              ]}
...
function SummaryRow({
  ...
  return (
    <View style={[summaryStyles.row, { borderBottomColor: tc.border }]}>
      <Text style={[summaryStyles.label, { color: tc.textSecondary }]}>
        {label}
      </Text>
      <Text
        style={[
          ...
          highlight && summaryStyles.highlightSize,
        ]}
      >
        {value}
      </Text>
...
const styles = StyleSheet.create({
  ...
  coachItem: {
    ...
    borderRadius: 12,
    borderWidth: 1,
    borderColor: "rgba(130,150,190,0.2)",
    backgroundColor: "rgba(130,150,190,0.04)",
  },
  coachItemSelected: {
    borderColor: "#162B5C",
