...
interface ManagerReservationsState {
  ...
  fetchManagerReservations: (refresh?: boolean) => Promise<void>;
  ...
  setPendingFilter: (filter: string | null) => void;
}

export const useManagerReservationsStore = create<ManagerReservationsState>((set, get) => ({
  reservations: [],
  allReservations: [],
  ...
  fetchManagerReservations: async (refresh = true) => {
    set({ isLoading: true, error: null });
    try {
      const res = await api.get<PaginatedResponse<Reservation>>('/reservations/manager/all', {
  ...
  updateReservationStatus: async (id: number, status: ReservationStatus) => {
    try {
      ...
      set((s) => ({
        ...
        allReservations: s.allReservations.map((r) => r.id === id ? { ...r, status } : r),
        currentReservation: s.currentReservation?.id === id
  ...
  setPendingFilter: (filter: string | null) => {
